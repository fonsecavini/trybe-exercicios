...
interface Person1 {
  ...
  class PhysicalPerson1 implements Person1 {
    ...
    constructor(name: string, cpf: string) {
      this._id = PhysicalPerson1.newId();
      this._name = name;
      this._cpf = cpf;
    }
  ...
  class LegalPerson1 implements Person1 {
    private static lastId = 0;
    private _name;
    private _id;
    private _cnpj;
  ...
  const pp0 = new PhysicalPerson1('John', '123456789');
  const pp1 = new PhysicalPerson1('Jenny', '987654321');
  const lp = new LegalPerson1('International Sales SA', '834729384723');
  
  const showIdentification = (person: Person1) => {
  ...
  */
  ...
  // Tipagem explícita
  const c2: Contract<LegalPerson1> = new Contract(lp); // Deixo explícito que lp é pessoa jurídica
